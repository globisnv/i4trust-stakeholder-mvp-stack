...
const _upsert_merge_policy = async function _upsert_merge_policy(req, res) {
  ...
  if (evidence_current != null)
  {
    ...
    for (let p_idx = 0; p_idx < evidence.policySets[0].policies.length; p_idx++) {
      ...
      const p_obj_prev_idx = -1;//p_types[p_resource.type].findIndex(obj => arrays_are_equal(obj.actions, p_actions) && arrays_are_equal(obj.attrs, p_resource.attributes));
      if (p_obj_prev_idx != -1) {
    ...
    /*for (let p_current_idx = 0; p_current_idx < evidence_current.policySets[0].policies.length; p_current_idx++) {
      ...
      if (p_types.hasOwnProperty(p_current_resource.type)) {
        ...
        const p_same_actions_idx = -1;//p_types[p_current_resource.type].findIndex(obj => arrays_are_equal(obj.actions, p_current_actions) && arrays_are_equal(obj.attrs, p_current_resource.attributes));
